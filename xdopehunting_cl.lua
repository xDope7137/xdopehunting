							...
							if PlyToAnimal < 2.0 then
								...
								if IsControlJustReleased(0, Keys['E']) then
									if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_KNIFE')  then
										...
										XD.Functions.Notify('You need to use the knife!', "error")
									end
								end
...
function SlaughterAnimal(AnimalId)
	...
	print('You slaughtered the animal and recieved' ..AnimalWeight.. 'kg of meat')

	TriggerServerEvent('xd:reward', AnimalWeight)

	DeleteEntity(AnimalId)
end

function SellItems()
	TriggerServerEvent('xd:sell')
end
